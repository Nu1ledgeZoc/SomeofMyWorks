...
class PhysicsInformedNN():
    def __init__(self, X, u, layers, layers1, Eoc):
        ...
        # self.pde_parameter = self.pde_parameter.cuda()
        # self.pde_parameter.requires_grad = True
    ...
    def train(self, nIter):
        ...
        for epoch in range(nIter):
            ...
            if self.k < 0.7:
                ...
                self.rp.data.clamp_(30.95, 31.15)

            if (self.k - 0.8).abs() + (self.rp - 30.927).abs() * 0.1 < 0.01:
                para_epoch = self.iter

            if epoch < 11900:
                loss_p = loss_f * 100 + loss_bcic * 10 + loss_u * 100
            ...
            if epoch % 100 == 0:
                print(
                    'It: %d, Loss: %.3e,u:%.3e,f:%.3e,bcic:%.3e  rohCp: %.3f, Kx: %.6f,para_epoch: %d' %  # , hc: %.6f
                    (
                        epoch,
                        loss.item(),
                        loss_u.item(),
        ...
        for epoch in range(nIter):
            ...
            bc_l = self.net_bc_l(self.x_bc_l, self.t, self.i, self.v)
            bc_r = self.net_bc_r(self.x_bc_r, self.t, self.i, self.v)
            loss_hc = torch.mean(bc_l ** 2) * 100 + torch.mean(bc_r ** 2) * 100 + torch.mean((self.u - u_pred) ** 2)
...
error_rohCp = np.abs(rohCp_value - 30.92775)
