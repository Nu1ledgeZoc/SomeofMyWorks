...
class ParaDNN(torch.nn.Module):
    def __init__(self, layers):
        super(ParaDNN, self).__init__()
...
class PhysicsInformedNN():
    def __init__(self, X, u, layers, layers1, layers2, Eoc):
        # boundary conditions
        ...
        # settings
        self.rohCp = torch.tensor([30.8], requires_grad=True).to(device)
        # self.Kx = torch.tensor([0.9], requires_grad=True).to(device)
    ...
    def net_para(self, x, t, i):
        para = self.dnn2(torch.cat([x, t, i], dim=1))
        k, h = para[:, 0], para[:, 1]
        return k, h
    ...
    def train(self, nIter):
        ...
        #     bc_r = self.net_bc_r(self.x_bc_r, self.t, self.i, self.v)
        #     loss_hc = np.linalg.norm(bc_l) + np.linalg.norm(bc_r) + loss_f + loss_u
        #     # loss_hc = torch.mean(bc_l ** 2) + torch.mean(bc_r ** 2) + torch.mean((self.u - u_pred) ** 2)
        #     self.optimizer_hc.zero_grad()
        #     loss_hc.backward()
    ...
    def predict(self, X):
        ...
        # rohCp = rohCp.detach().cpu().numpy()
        Kx = Kx.detach().cpu().numpy()
        hc = hc.detach().cpu().numpy()
        return u, f, q, Kx, hc
...
# training
model = PhysicsInformedNN(X_u_train, u_train, layers, layers1, layers2, Eoc_train)
model.train(8000)
...
# evaluations
u_pred, f_pred, q_pred, Kx_p, hc_p = model.predict(X_star)
...
Kx_value = np.mean(Kx_p ** 2)
hc_value = np.mean(hc_p ** 2)
